...
$packages["main"] = (function() {
	var $pkg = {}, $init, js, strconv, funcType, sliceType, ptrType, funcType$1, main, createBoard, toggleFound, toggleFoundness, refreshBoard, updateProgressBar, restartGame;
	js = $packages["github.com/gopherjs/gopherjs/js"];
	...
	main = function() {
		$global.document.getElementById($externalize("reset", $String)).onclick = $externalize(restartGame, funcType);
		createBoard();
		refreshBoard();
	};
	createBoard = function() {
		var _i, _ref, document, elem, name, usStates;
		console.log("create board");
		usStates = new sliceType(["Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", "Florida", "Georgia", "Hawaii", "Idaho", "Illinois", "Indiana", "Iowa", "Kansas", "Kentucky", "Louisiana", "Maine", "Maryland", "Massachusetts", "Michigan", "Minnesota", "Mississippi", "Missouri", "Montana", "Nebraska", "Nevada", "New Hampshire", "New Jersey", "New Mexico", "New York", "North Carolina", "North Dakota", "Ohio", "Oklahoma", "Oregon", "Pennsylvania", "Rhode Island", "South Carolina", "South Dakota", "Tennessee", "Texas", "Utah", "Vermont", "Virginia", "Washington", "West Virginia", "Wisconsin", "Wyoming"]);
		...
		while (true) {
			...
			elem = document.createElement($externalize("div", $String));
			elem.className = $externalize("us_state", $String);
			elem.id = $externalize(name, $String);
			elem.textContent = $externalize(name, $String);
			elem.onclick = $externalize(toggleFound, funcType$1);
			document.getElementById($externalize("content", $String)).appendChild(elem);
			_i++;
		}
	};
	toggleFound = function(e) {
		var e;
		toggleFoundness(e);
		refreshBoard();
	};
	toggleFoundness = function(e) {
		var e, item, name;
		name = e.srcElement.id;
		item = $global.localStorage.getItem(name);
		if (item === undefined || item === null) {
			console.log("add", name, "to localStorage");
			$global.localStorage.setItem(name, $externalize(true, $Bool));
		} else {
			console.log("remove", name, "from localStorage");
			$global.localStorage.removeItem(name);
		}
	};
	refreshBoard = function() {
		var count, elem, found, i, item, length, nodeList, stored;
		console.log("refresh board");
		nodeList = $global.document.querySelectorAll($externalize(".us_state", $String));
		...
		while (true) {
			...
			item = $global.localStorage.getItem(elem.id);
			stored = !(item === undefined || item === null);
			if (stored) {
				count = count + (1) >> 0;
			}
			found = !!(elem.classList.contains($externalize("found", $String)));
			if (stored && !found) {
				console.log("add \"found\" class to", elem.id);
				elem.classList.add($externalize("found", $String));
			} else if (!stored && found) {
				console.log("remove \"found\" class from", elem.id);
				elem.classList.remove($externalize("found", $String));
			}
			i = i + (1) >> 0;
		}
	...
	updateProgressBar = function(count) {
		...
		if (count >= 0 && count <= 50) {
			...
			doneElem.textContent = $externalize(strconv.Itoa(count), $String);
